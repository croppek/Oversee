//wyświetlanie aktualnego adresu URL = console.log(window.location.protocol + "//" + window.location.host + "" + window.location.pathname);

/*Funkcja do cachowania dużej ilości selektorów
function Selector_Cache() {
	var collection = {};

	function get_from_cache( selector, reset ) {
		if ( collection[ selector ] === undefined || reset === true) 
        {
...
var mobile_elements_position = (function(){
    ...
    var $toggle_menu = $('#toggle_menu_btn'),
        ...
        $login_input_group = $('.login_input_group'),
    ...
    function _positionElements(){
        ...
        if($toggle_menu.css('display') == 'none')
        {
            ...
            $change_password.add($error_alert).add($login_input_group).add($admin_panel).add($add_new_users).css('width', '75%');
        ...
        else
        {
            ...
            $change_password.add($error_alert).add($login_input_group).add($admin_panel).add($add_new_users).css('width', '95%');
...
var change_page_language = (function(){
    ...
    function _change_language($btn){
        ...
        $.post("php/moduly/set_page_language.php", {language: lang}, function(data){
...
var add_item_to_db = (function(){
    
    var category;
    
    //#### Cache DOM
    var modal_content = $('#modal_addtodb_content'),
        modal_dialog = $('#addtodb_modal_dialog'),
        toggle_menu = $('#toggle_menu_btn'),
        get_category = $('#add_todb_categories_select'),
        add_to_db_btn = $('#add_todb_category');
    
    //#### Bind events and init
    add_to_db_btn.on('click', _add_to_db_content);
    modal_content.on('click', '#add_item_to_db', _add_item_to_db);

    //#### Functions
    
    //wyświetlenie panelu dodawania przedmiotu
    function _add_to_db_content()
    {
        add_to_db_btn.attr("disabled", "disabled");
        ...
        {    
            ...
            {
                modal_dialog.animate({width: '95%'});
                modal_content.animate({marginBottom: '50px'}, 500, function(){
            ...
            {
                modal_dialog.animate({width: '80%'});
                modal_content.animate({marginBottom: '320px'}, 500, function(){
        ...
        setTimeout(function(){add_to_db_btn.removeAttr("disabled")}, 1500);
    }
    
    //dodanie przedmiotu do bazy danych
    function _add_item_to_db()
    {
        setTimeout(function(){
...
var remove_item_from_db = (function(){
    
    $('#remove_item_from_db_btn').on('click', _remove_item);
...
var remove_comment_from_db = (function(){
    
    $('.remove_comment_from_db_btn').on('click', _remove_comment);
...
var edit_item_info = (function(){
    
    //#### Cache DOM (global)
    var edit_item_btn = $('.edit_item_info_btn'),
        category = $('#item_category_holder').text(),
        page_blend = $('#page_blend'),
        toggle_menu = $('#toggle_menu_btn');
        
    //#### Bind events
    edit_item_btn.on('click', function(){ _edit_info( $(this) ) });
    
    function _edit_info($that)
    {
        $that.attr("disabled", "disabled");

        var header =  $that.parent().data('header');

        var element = $that.parent().parent().parent().parent();
        var element_content = $that.parent().parent().next();
        ...
        page_blend.fadeIn(250, function(){
            ...
            if(toggle_menu.css('display') == 'block' || $('body').width() <= 974)
            {
            ...
            $(element_content).on('click', '#close_edit_btn', function(){
                ...
                setTimeout(function(){edit_item_btn.removeAttr("disabled")}, 500);
...
var add_new_comment = (function(){
    ...
    var error_alert = $('#error_alert'),
        add_comment_btn = $('#add_comment_to_db'),
    ...
    checkbox.change(_show_nofitication_settings);
    add_comment_btn.on('click', _add_comment);
    
    function _show_nofitication_settings() 
    {
        if(checkbox.is(':checked'))
        {
            inform_specialist = true;
            notifications.slideDown(100);
        }
        else
        {
        ...
        else
        {
            ...
            if(inform_specialist == true)
            {
                ...
                if(specialization == 'brak')
                {
                    ...
                    setTimeout(function(){add_comment_btn.removeAttr("disabled")}, 1500);
                    return;
                }
                else
                {
